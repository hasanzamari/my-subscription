...
code = """import json, os, time, subprocess, re, base64
...
def main():
    ...
    for h, info in db.items():
        ...
        try:
            if not lt or datetime.fromisoformat(lt) < cutoff: untested.append((h, info))
        except: untested.append((h, info))
...
with open("Core/health_checker.py", "w", encoding="utf-8") as f:
    f.write(code)
print("✅ SUCCESS: Core/health_checker.py created perfectly!")
