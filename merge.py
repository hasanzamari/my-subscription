...
from Core import source_guardian # ✅ اضافه کردن نگهبان سورس
...
def main():
    ...
    # پاکسازی اولیه سورس‌ها قبل از دانلود
    ...
    # ✅ جمع‌آوری آمار برای نگهبان سورس
    source_results = {}
    for url, content in raw.items():
        ok = bool(content)
        count = content.count("\n") + 1 if content else 0
        
        # تخمین تعداد کانفیگ‌های معتبر از این سورس (ساده‌سازی شده)
        # در سیستم شما، valid یا final بهتر است، اما اینجا بر اساس content تخمین می‌زنیم
        # اگر سیستم شما دقیق‌تر است، می‌توانید تعداد واقعی را از result بگیرید
    ...
    # ✅ ارزیابی و بن کردن سورس‌های خراب
