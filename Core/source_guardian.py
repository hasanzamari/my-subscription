...
ACTIVE_SOURCES_FILE = "Sources/sources.txt" # مسیر فایل سورس‌های فعال شما
...
def save_blacklist(blacklist):
    ...
    with open(BLACKLIST_FILE, "w", encoding="utf-8") as f:
        for url in sorted(list(blacklist)):
            f.write(url + "\n")

def normalize_url(url):
    """حذف فاصله‌ها و اسلش انتهایی برای جلوگیری از تکرار"""
    return url.strip().rstrip("/")

def evaluate_sources(sources_results):
    """
    sources_results: دیکشنری { "url": {"success": bool, "valid_count": int} }
    """
    health = load_health()
    ...
    for url, result in sources_results.items():
        ...
        if norm_url in blacklist:
            continue # اگر در لیست سیاه است، نادیده بگیر
        ...
        # شرط سلامت: هم دانلود موفق باشد هم حداقل ۵ کانفیگ معتبر داشته باشد
        if result.get("success") and result.get("valid_count", 0) >= 5:
            entry["success"] += 1
            entry["fail"] = 0 # ریست کردن شکست‌ها چون موفق بوده
            entry["total_valid"] += result.get("valid_count", 0)
        else:
            entry["fail"] += 1
            entry["success"] = 0 # ریست کردن موفقیت‌ها
            
        # اگر ۳ بار متوالی شکست خورد، به لیست سیاه برود
        if entry["fail"] >= 3:
    ...
    return health, blacklist

def clean_active_sources():
    """پاکسازی فایل سورس‌های فعال از موارد تکراری و لیست سیاه"""
    if not os.path.exists(ACTIVE_SOURCES_FILE):
        return
    ...
    log(f"🧹 Cleaned active sources: {len(lines)} -> {len(clean_urls)} (Removed duplicates & blacklisted)")
