...
def test_all_configs(db_path: str = DB_FILE, max_configs: int = MAX_CONFIGS_PER_RUN, sync_db: bool = True) -> Dict[str, Any]:
    try:
        if not os.path.exists(db_path):
            log(f"❌ فایل دیتابیس پیدا نشد: {db_path}")            return {"error": "Database not found"}
        ...
        summary = {
            ...
            "failed": failed_count,
            "success_rate": round(success_count / tested * 100, 2) if tested > 0 else 0,            "avg_latency": round(avg_latency, 2),
...
if __name__ == "__main__":
    ...
    test_all_configs(max_configs=args.max, sync_db=not args.no_sync)
