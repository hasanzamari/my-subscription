...
DB_FILE, HEALTH_FILE, WORKERS, TIMEOUT, CHUNK = "database/database.json", "database/config_health.json", 50, 3, 3000
def extract_addr(cfg):
    try:
        cfg = cfg.strip()
        if cfg.startswith("vmess://"): return json.loads(base64.b64decode(cfg[8:]).decode()).get("add")
        for p in ["vless://", "trojan://", "ss://", "hy2://", "hysteria://", "tuic://"]:
            if cfg.startswith(p):
                m = re.search(rf"{re.escape(p)}[^@]+@([^:]+):", cfg)
                if m: return m.group(1)
        if cfg.startswith("ssr://"): return base64.b64decode(cfg[6:]).decode().split(":")[0]
    except: pass
    return None
def worker(args):
    h, info = args
    addr = extract_addr(info.get("config", ""))
    ...
    try:
        ...
        if r.returncode == 0:
            ...
            return h, True, ms, None
        return h, False, None, "fail"
    except: return h, False, None, "timeout"
def main():
    if not os.path.exists(DB_FILE): return log("DB not found")
    with open(DB_FILE, "r", encoding="utf-8") as f: db = json.load(f)
    cutoff = datetime.now() - timedelta(hours=24)
    untested = [(h, info) for h, info in db.items() if not info.get("last_test") or datetime.fromisoformat(info.get("last_test")) < cutoff]
